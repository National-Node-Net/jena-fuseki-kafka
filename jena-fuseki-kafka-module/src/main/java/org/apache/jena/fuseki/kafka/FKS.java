...
public class FKS {
     ...
     */
    public static void addConnectorToServer(KConnectorDesc conn, DataState dataState, FKBatchProcessor batchProcessor) {
            ...
            Deserializer<RequestFK> reqDer = new DeserializerActionFK())
        {
            ...
            // ASYNC
            startTopicPoll(batchProcessor, consumer, dataState);
        }
    }
    ...
    public static Pair<ActionProcessor, DatasetGraph> findActionProcessorDataset(FusekiServer server, String uriPath) {
        ...
        String datasetName = path.datasetName;
        String endpointName = path.endpointName;
    ...
    private static void startTopicPoll(FKBatchProcessor requestProcessor, Consumer<String, RequestFK> consumer, DataState dataState) {
        Runnable task = () -> topicPoll(requestProcessor, consumer, dataState);
        threads.submit(task);
    }

    /** Polling task loop. */
    @SuppressWarnings("java:S2189") // SonarQube rule "Add an end condition to this loop. The design approach chosen was an infinite loop (to be reviewed)"
    private static void topicPoll(FKBatchProcessor requestProcessor, Consumer<String, RequestFK> consumer, DataState dataState) {
        for( ;; ) {
            try {
                ...
                FmtLog.debug(LOG, ex, "[%s] Unexpected Exception %s", dataState.getTopic(), dataState);
            }
        }
    }
